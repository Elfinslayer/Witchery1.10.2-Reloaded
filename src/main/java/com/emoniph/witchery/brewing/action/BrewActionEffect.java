...
public abstract class BrewActionEffect extends BrewAction
{
  ...
  public final void applyRitualToEntity(World world, EntityLivingBase targetEntity, ModifiersRitual ritualModifiers, ModifiersEffect modifiers, ItemStack stack)
  {
    if (!disableEntityTarget) {
      doApplyRitualToEntity(world, targetEntity, ritualModifiers, modifiers, stack);
      modifiers.reset();
    }
  }
  ...
  {
    if (!disableEntityTarget) {
      doApplyToEntity(world, targetEntity, modifiers, stack);
      modifiers.reset();
    }
  }
  ...
  {
    if (!disableBlockTarget) {
      doApplyToBlock(world, x, y, z, side, radius, modifiers, stack);
      modifiers.reset();
    }
  }
  ...
  {
    if (!disableBlockTarget) {
      doApplyRitualToBlock(world, x, y, z, side, radius, ritualModifiers, modifiers, stack);
      modifiers.reset();
    }
  }
